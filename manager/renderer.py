...
def build_xray_config(devices: list[Device], settings: Settings) -> dict:
    ...
    return {
        ...
        "routing": {"domainStrategy": "AsIs", "rules": [{"type": "field", "protocol": ["bittorrent"], "outboundTag": "block"}, {"type": "field", "port": "25,465,587", "network": "tcp", "outboundTag": "block"}]},
...
def build_hysteria_config(devices: list[Device], settings: Settings) -> dict:
    ...
    return {
        ...
        "acl": {"inline": ["reject(all, tcp/25)", "reject(all, tcp/465)", "reject(all, tcp/587)", "reject(all, tcp/6881-6999)", "reject(all, udp/6881-6999)", "allow(all)"]},
